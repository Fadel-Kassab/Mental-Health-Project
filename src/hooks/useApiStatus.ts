...
export function useApiStatus<Call extends ApiCall>({
  ...
  const [isLoading, setIsLoading] = useState<ReturnType["isLoading"]>(false)
  const [validationErrors, setValidationErrors] = useState<ReturnType["validationErrors"]>()
  const [errorMessage, setErrorMessage] = useState<ReturnType["errorMessage"]>()
  ...
  useEffect(() => {
    if (fireOnMount && defaultParams) fire(...defaultParams)
  }, [])
